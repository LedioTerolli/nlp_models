...
class LLDA:
    ...
    def set_corpus(self, labelset, corpus, labels):
        ...
        self.labels = numpy.array(
            [self.complement_label(label) for label in labels])
        ...
        for m, doc, label in zip(range(M), self.docs, self.labels):
            N_m = len(doc)
            z_n = [numpy.random.multinomial(
                1, label / label.sum()).argmax() for x in range(N_m)]
    ...
    def inference(self):
        ...
        for m, doc, label in zip(range(len(self.docs)), self.docs, self.labels):
            for n in range(len(doc)):
                ...
                p_z = label * (self.n_z_t[:, t] + self.beta) / \
                    denom_b * (self.n_m_z[m] + self.alpha) / denom_a
